...
# ESRD Group Performance {#care-esrdgrp}
...
#| label: data_dict
#| echo: false
readr::read_csv(here::here("data", "esrd_group_perf.csv"))
```
...
resp <- quick("esrd_group_perf", limit = 5000) |> 
  mtt(
    ...
    HDR_DNM_MY = as.numeric(HDR_DNM_MY),
    HDR_NUM_MY = as.numeric(HDR_NUM_MY),
    HD_NUM_MY = as.numeric(HD_NUM_MY),
    SD_NUM_MY = as.numeric(SD_NUM_MY),
    NOCD_NUM_MY = as.numeric(NOCD_NUM_MY),
    TR_MY_ACHSCR = as.numeric(TR_MY_ACHSCR),
    TR_MY_IMPSCR = as.numeric(TR_MY_IMPSCR),
    TWR_DNM_MY = as.numeric(TWR_DNM_MY),
    TWR_NUM_MY = as.numeric(TWR_NUM_MY),
    LDT_DNM_MY = as.numeric(LDT_DNM_MY),
    LDT_NUM_MY = as.numeric(LDT_NUM_MY),
    BENECNT_LDT_MY = as.numeric(BENECNT_LDT_MY),
    HEI_HDR = as.numeric(HEI_HDR),
    HDR_DELIS_DIFF = as.numeric(HDR_DELIS_DIFF),
    HDR_DELIS_MY = as.numeric(HDR_DELIS_MY),
    HDR_DNM_DELIS_MY = as.numeric(HDR_DNM_DELIS_MY),
    HDR_NUM_DELIS_MY = as.numeric(HDR_NUM_DELIS_MY),
    HEI_TR = as.numeric(HEI_TR),
    TR_DELIS_DIFF = as.numeric(TR_DELIS_DIFF),
    TR_DELIS_MY = as.numeric(TR_DELIS_MY),
    TWR_DNM_DELIS_MY = as.numeric(TWR_DNM_DELIS_MY),
    TWR_NUM_DELIS_MY = as.numeric(TWR_NUM_DELIS_MY),
    LDT_DNM_DELIS_MY = as.numeric(LDT_DNM_DELIS_MY),
    LDT_NUM_DELIS_MY = as.numeric(LDT_NUM_DELIS_MY),
    MPS = as.numeric(MPS),
    PPA = as.numeric(PPA),
