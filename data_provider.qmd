...
```{r}
weburl::url_parse("https://data.cms.gov/provider-data/api/1/datastore/query/7t8x-u3ir/0?offset=0") |> 
...
```{r}
weburl::url_parse("https://data.cms.gov/provider-data/api/1/datastore/query/7t8x-u3ir/0?offset=2000") |> 
  mutate(across(where(is.character),  ~ na_if(.x, ""))) |> 
  remove_all_na() |> 
  purse()
```
...
```{r}
weburl::url_parse(
"https://data.cms.gov/provider-data/api/1/datastore/query/4pq5-n9py/0?conditions[0][property]=overall_rating&conditions[0][value]=5&conditions[0][operator]=<") |> 
  mutate(across(where(is.character),  ~ na_if(.x, ""))) |> 
  remove_all_na() |> 
  purse()
```
...
```{r}
nh <- request("https://data.cms.gov/provider-data/api/1/datastore/query/4pq5-n9py/0?limit=2000&conditions[0][property]=overall_rating&conditions[0][value]=5&conditions[0][operator]=<") |> 
  req_perform() |> 
...
```{r}
ex <- request("https://data.cms.gov/provider-data/api/1/datastore/query/4pq5-n9py/0") |> 
  req_perform() |> 
  resp_body_json(simplifyVector = TRUE)
...
```{r}
ex <- request("https://data.cms.gov/provider-data/api/1/datastore/query/4pq5-n9py/0?conditions[0][property]=overall_rating&conditions[0][value]=5&conditions[0][operator]==") |> 
  req_perform() |> 
  resp_body_json(simplifyVector = TRUE)
...
```{r}
ex <- request("https://data.cms.gov/provider-data/api/1/metastore/schemas/dataset/items") |> 
  req_perform() |> 
  resp_body_json(simplifyVector = TRUE)
