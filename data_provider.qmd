...
```
   ...
   * [Provider-Data API Documentation](https://data.cms.gov/provider-data/docs)

```{r}
dataset_provider <- request("https://data.cms.gov/provider-data/api/1/metastore/schemas/dataset/items") |>
  req_perform() |>
...
dataset_provider |> purse()

distribution_provider <- dataset_provider |> 
  select(distribution) |> 
  unnest(distribution)

distribution_provider |> purse()
```
