...
open_identifier_url <- function(x) {
  paste0("https://openpaymentsdata.cms.gov",
         "/api/1/datastore/query/", x, "/0")
}
...
ex_id <- open_payments_general(open) |> 
    _[["endpoints"]] |> 
  sbt(year == 2021) |> 
  _[["identifier"]]
...
ex_req <- create_request_open(
  ex_id,
  c(covered_recipient_npi = 1023630738)
)
```
