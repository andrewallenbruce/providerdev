---
editor_options: 
   chunk_output_type: console
---

```{r}
#| label: setup-common-01
#| include: false
#| warning: false
source("includes/_common.R")
```

# Joint Model MSAs {#care-joint}

The __Comprehensive Care for Joint Replacement Model: Metropolitan Statistical Areas (MSAs)__ dataset presents MSAs that are participating in the __CMS Innovation Center Comprehensive Care for Joint Replacement Model__, a model to support better and more efficient care for beneficiaries undergoing the most common inpatient surgery for Medicare beneficiaries: hip and knee replacements. Participation in this model is designated by geographic area, specifically MSAs. The information contained in the dataset can include MSA identification number, MSA geographic name and associated county or counties.

## Metadata

```{r}
#| label: s7_object
#| echo: false
end <- care_endpoint("cpc_joint")

end@metadata[c(time, link)] |> print_ls()
```

### Resources

```{r}
#| label: resources
#| echo: false
end |> list_resources()
```


### Dictionary

```{r}
#| label: data_dict
#| echo: false
# browse_dictionary(end)
new_tbl(
  field = c(
    "MSA", 
    "MSA Title", 
    "Counties"
  ),
  description = c(
    "Metropolitan Statistical Area (MSA) identification number, as published by the Office of Management and Budget.", 
    "Geographic name associated with the respective Metropolitan Statistical Area (MSA) identification number, as published by the Office of Management and Budget.", 
    "Counties associated with the respective Metropolitan Statistical Area, as published by the Office of Management and Budget."
  )
)
```

## Data

```{r}
#| label: http_request
#| message: true
resp <- quick("cpc_joint", limit = 5000)

resp
```

## Overview

```{r}
#| label: codebook
#| echo: false
resp |> collapse::descr()
```
