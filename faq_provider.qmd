...
provider_data() |>
  reframe(
    ...
    dates   = issued,
    released,
    links   = distribution_downloadURL,
    website = landingPage) |>
  arrange(id, title) |>
  gt() |>
  tab_header(title    = md("**Provider Data Catalog API**: Datasets Available")) |> 
  opt_all_caps() |>
  opt_table_font(font = google_font(name = "Rubik")) |>
  fmt_date(columns    = c(dates, released)) |>
  fmt_url(columns     = links, show_underline = TRUE, label = "Download") |>
  fmt_url(columns     = website, show_underline = TRUE, label = "Landing Page") |>
  tab_style(style     = css(position = "sticky", top = "-1em", `z-index` = 10), 
            locations = cells_column_labels()) |>
  tab_style(style     = cell_text(weight = "lighter", size = "small"),
            locations = cells_body(columns = c(description, dates, released))) |>
  tab_style(style     = cell_text(weight = "bold", size = "small", 
                                  font = google_font(name = "Fira Code")), 
            locations = cells_body(columns = c(links, website))) |>
  tab_style(style     = cell_text(weight = "bold", size = "small"),
            locations = cells_body(columns = title)) |>
  cols_merge(columns  = c(dates, released), 
             pattern  = "<b>ISSUED</b>: {1} <br> <b>RELEASED</b>: {2}") |>
  cols_merge(columns  = c(links, website), 
             pattern  = "{1} <br> {2}") |>
  tab_options(container.height                  = px(700),
              ...
              heading.background.color          = "gray60",
...
ds_post <- request("https://data.cms.gov/provider-data/api/1/datastore/query/f93da3a3-400b-5efe-960d-c1d72b1d4f65") |> 
  req_method("POST") |> 
